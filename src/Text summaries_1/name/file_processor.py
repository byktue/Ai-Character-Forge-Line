import os
from api_request import extract_person_info

def format_person_info(person_info):
    output = "对于文本中主要人物的名称及别名信息：\n"
    output += f"  人物信息：{', '.join(person_info)}\n"
    return output
...
def process_file(DEEPSEEK_API_URL, DEEPSEEK_API_KEY, DEEPSEEK_MODEL, input_file_path, output_file_path, encoding):
    try:
        with open(input_file_path, 'r', encoding=encoding) as file:
            text = file.read()
        person_info = extract_person_info(DEEPSEEK_API_URL, DEEPSEEK_API_KEY, DEEPSEEK_MODEL, text)
        if person_info:
            # 只有在提取到人物信息时，才创建输出文件
            formatted_person_info = format_person_info(person_info)
            with open(output_file_path, 'w', encoding=encoding) as output_file:
                output_file.write(formatted_person_info)
            print(f"已处理并保存文件: {input_file_path} -> {output_file_path}")
        else:
            print(f"未提取到有效人物信息，跳过文件: {input_file_path}")
