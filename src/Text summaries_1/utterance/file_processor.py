import os
from api_request import extract_role_dialogues

def format_role_dialogues(dialogues):
    output = "对于文本中角色的话语信息：\n"
    output += f"  角色话语：{', '.join(dialogues)}\n"
    return output
...
def process_file(DEEPSEEK_API_URL, DEEPSEEK_API_KEY, DEEPSEEK_MODEL, input_file_path, output_file_path, encoding):
    try:
        with open(input_file_path, 'r', encoding=encoding) as file:
            text = file.read()
        dialogues = extract_role_dialogues(DEEPSEEK_API_URL, DEEPSEEK_API_KEY, DEEPSEEK_MODEL, text)
        if dialogues:
            # 只有在提取到角色话语信息时，才创建输出文件
            formatted_dialogues = format_role_dialogues(dialogues)
            with open(output_file_path, 'w', encoding=encoding) as output_file:
                output_file.write(formatted_dialogues)
            print(f"已处理并保存文件: {input_file_path} -> {output_file_path}")
        else:
            print(f"未提取到有效角色话语信息，跳过文件: {input_file_path}")
