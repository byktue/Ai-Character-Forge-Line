import os
from api_request import extract_appearance_features
...
def process_file(DEEPSEEK_API_URL, DEEPSEEK_API_KEY, DEEPSEEK_MODEL, input_file_path, output_file_path, encoding):
    try:
        with open(input_file_path, 'r', encoding=encoding) as file:
            text = file.read()
        features = extract_appearance_features(DEEPSEEK_API_URL, DEEPSEEK_API_KEY, DEEPSEEK_MODEL, text)
        if features:
            # 只有在提取到外貌特征信息时，才创建输出文件
            ...
            print(f"未提取到有效外貌特征信息，跳过文件: {input_file_path}")
