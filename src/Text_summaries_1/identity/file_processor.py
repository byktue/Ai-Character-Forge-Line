import os
from api_request import extract_character_roles

def format_character_roles(roles):
    output = "对于文本中主要人物的角色身份信息：\n"
    output += f"  角色身份信息：{', '.join(roles)}\n"
    return output
...
def process_file(DEEPSEEK_API_URL, DEEPSEEK_API_KEY, DEEPSEEK_MODEL, input_file_path, output_file_path, encoding):
    try:
        with open(input_file_path, 'r', encoding=encoding) as file:
            text = file.read()
        roles = extract_character_roles(DEEPSEEK_API_URL, DEEPSEEK_API_KEY, DEEPSEEK_MODEL, text)
        if roles:
            # 只有在提取到角色身份信息时，才创建输出文件
            formatted_roles = format_character_roles(roles)
            with open(output_file_path, 'w', encoding=encoding) as output_file:
                output_file.write(formatted_roles)
            print(f"已处理并保存文件: {input_file_path} -> {output_file_path}")
        else:
            print(f"未提取到有效角色身份信息，跳过文件: {input_file_path}")
